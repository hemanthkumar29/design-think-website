...
import { Award, Save, Percent, ChevronDown, ChevronUp, Users, Crown } from 'lucide-react';
import StarRating from '@/components/ui/StarRating';
import { fetchTeamsForAdmin, updateTeamProgressInDB } from '@/services/adminRealtimeService';
...
const AdminDashboard = () => {
  ...
  const handleProgressSave = async (teamId: number) => {
  ...
  // Component for individual team rating management
  const TeamRatingManager: React.FC<{ team: AdminTeamData }> = ({ team }) => {
    const { ratings, isLoading: ratingsLoading, isUpdating, updateLeader, updateMember } = useTeamRatings(team.id);
      ...
      <div className="py-4 px-2">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h4 className="text-lg font-semibold text-foreground">
            Team {team.id} Members - Performance Rating ({(ratings?.members.length || 0) + 1} Total)
          </h4>
        </div>
        ...
        <div className="grid gap-4">
          ...
          <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg p-4 border-2 border-yellow-200">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <StarRating 
                  rating={ratings?.leaderRating || 0}
                  onChange={updateLeader}
                  size="md"
                  isLoading={isUpdating[`leader_${team.id}`]}
                />
              </div>
            </div>
          </div>
          ...
          ) : ratings && ratings.members.length > 0 ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <Users className="h-4 w-4 text-blue-500" />
                <span className="text-sm font-medium text-muted-foreground">
                  Team Members ({ratings.members.length})
                </span>
              </div>
              {ratings.members.map((member, index) => (
                <div 
                  key={member.id}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <StarRating 
                        ...
                        onChange={(rating) => updateMember(member.id, rating)}
                        size="md"
                        isLoading={isUpdating[member.id]}
                      />
                    </div>
                  </div>
                </div>
                ...
                {teams.map((team) => (
                  <React.Fragment key={team.id}>
                    <TableRow className="group">
                      ...
                      <TableCell>
                        <Button
                          onClick={() => handleProgressSave(team.id)}
                          size="sm" 
                      ...
                      <TableCell>
                        ...
                        >
                          ...
                          Rate Members ({(teams.find(t => t.id === team.id)?.members.length || 0) + 1})
                        </Button>
                      </TableCell>
                    </TableRow>
                    {expandedTeams[team.id] && (
