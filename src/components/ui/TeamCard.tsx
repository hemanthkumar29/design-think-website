...
const TeamCard: React.FC<TeamCardProps> = memo(({ id, name, progress: initialProgress, className }) => {
  ...
  const { ratings, isLoading } = useTeamRatings(id);
  ...
  const progress = ratings?.teamId ? initialProgress : initialProgress;
            ...
            {ratings.members.length > 0 && (
              <div className="mt-2">
                ...
                <ul className="text-xs opacity-70 space-y-0.5 pl-4">
                  {ratings.members.slice(0, 3).map((member) => (
                  ...
                  {ratings.members.length > 3 && (
                    <li className="text-xs opacity-50">+{ratings.members.length - 3} more</li>
                  )}
                </ul>
              </div>
            )}
